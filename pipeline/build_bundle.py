...
def ingest_shelters(paths, regions):
    """무더위쉼터 실좌표 파일(csv|json) → 쉼터 리스트. 좌표→최근접 생활권으로 귀속."""
    out = []
    for path in paths:
        try:
            rows = []
            if path.lower().endswith(".json"):
                with open(path, "r", encoding="utf-8-sig") as f:
                    j = json.load(f)
                rows = j if isinstance(j, list) else (j.get("records") or j.get("data") or [])
                if rows and isinstance(rows[0], dict):
                    header = list(rows[0].keys())
                    getr = lambda row, i: row.get(header[i]) if 0 <= i < len(header) else None
            else:
                with open(path, "r", encoding="utf-8-sig", newline="") as f:
            ...
            for k, row in enumerate(rows):
                ...
                if bcode is None or best > 0.25 ** 2:
                    continue
                nm = getr(row, i_nm) if i_nm >= 0 else None
                out.append({
                    "id": f"{bcode}-RS{k:04d}",
                    "name": str(nm).strip() if nm else f"무더위쉼터 {k+1}",
                    "lat": round(la, 5), "lng": round(lo, 5),
                    "regionCode": bcode,
                    "type": (str(getr(row, i_ty)).strip() if i_ty >= 0 and getr(row, i_ty) else "쉼터"),
                    "capacity": None, "operatingHours": "09:00~18:00", "isOutdoor": False,
                })
...
def main():
    ...
    real_shelters = ingest_shelters(raw["shelter"], jeonbuk_regions + seoul_regions) if raw["shelter"] else []
    ...
    # ── 쉼터(합성 명시, H2) ──
    if real_shelters:
        notes.append(f"무더위쉼터 실좌표 {len(real_shelters)}개소 적용(전량 파일 기반).")
    else:
